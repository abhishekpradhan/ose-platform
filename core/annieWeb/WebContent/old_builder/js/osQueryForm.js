var FEATURE_GROUPS = ['brand','model','mpix','zoom','price','other'];
...
function updateFeatures(){
	...
	for (var i = 0 ; i < FEATURE_GROUPS.length; i++){
		featureString += getAllFeaturesInGroup(FEATURE_GROUPS[i],false) + " ";
	}
...
function searchLogistic(){
	...
	var featureString = "";
	for (var i = 0 ; i < FEATURE_GROUPS.length; i++){
		var query = getAllFeaturesInGroup(FEATURE_GROUPS[i],true) ;
		if (query == "")
			query = "%Null()";		
		featureString += query + " ";
	}
	...
	parent.frameResult.searchLogistic(gIndexId, featureString);
}
