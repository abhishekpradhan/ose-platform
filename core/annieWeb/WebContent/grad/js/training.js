//Global variables
var myDataTable;
var SEARCH_SERVLET_URL = "/annieWeb/AnnotationSearchServlet?";
var GET_QUERIES_SERVLET_URL = "/annieWeb/GetTrainingQueries";  
var GET_INDICES_SERVLET_URL = "/annieWeb/GetIndices";  
var FEEDBACK_SERVLET_URL = "/annieWeb/TrainingFeedbackServlet";  
var TAGGING_SERVLET = "/annieWeb/TaggingServlet";
 
var gFullURL = parent.document.location + ""; //convert to string
var gQueryString = gFullURL.indexOf('?')==-1?"":gFullURL.substring(gFullURL.indexOf('?') + 1);
var gEnableImage = gQueryString.indexOf('image=yes') != -1; 
...
var TitleTemplate = new Template(
   '<h2 class="ostitle"> \
    <div onclick="loadTags(#{DOCID})"><a target="frameWebpage" href="#{CACHE_URL}">#{TITLE}</a></div></h2>\
    <table border="0" cellpadding="0" cellspacing="0"> \
        <tr> \
            <td class="j">#{DESCRIPTION}<br> <span class="a">#{DOMAIN} - </span> \
            <nobr> \
            <a class="fl" target="_blank" href="#{URL}"> Original</a> - \
            <a class="fl" target="_blank" href="#{CACHE_URL}&nofilter"> Cached</a> \
            </nobr></font>\
            </td> \
        </tr> \
    </table>');
...
var ImageTemplate = new Template(
    "<img width=100 src=\"/imageServer/ImageServlet?url=#{URL}\"/>"
);
...
function renderMyPageTable() {
	...
	var formatImage = function(elCell, oRecord, oColumn, sData) { 
        var url = oRecord.getData("Url");
		elCell.innerHTML = ImageTemplate.evaluate( {"URL" : url } ) ;
    };
    ...
    YAHOO.example.ClientPagination = new function() { 
        ...
        {key: "DocId", label: "DocID", formatter: gEnableImage?formatImage:null, sortable: true, width : "10px"},
        ...
        this.myDataSource = new YAHOO.util.DataSource(SEARCH_SERVLET_URL);
...
function selectField(){
    var selectBox = $("selField");
    var selectedOption = selectBox.options[selectBox.selectedIndex]
    gFieldId = selectedOption.value;
    logMessage("Select field: " + gFieldId);    
}
...
function selectQuery(){
    ...
    logMessage("Query : " + selectedOption.value);
    for (key in query){
...
function loadQueries(){
	var domainId = $("inpDomainId").value;
	if (gFieldId == -1){
		alert("invalid field id " + gFieldId);
		return;
	}
	options = { method:'get',
				parameters : {domainId : domainId, fieldId : gFieldId}, 
...
function addNewText(){
	if (gFieldId == -1 || gIndexId == -1 || gTaggingDocId == -1){
		alert("bad fieldid/indexid/docid");
		return;
	}
	var input = prompt("Please enter new tag for field " + gFieldId,"");
	if (input != null){
		addTag(gIndexId, gTaggingDocId, gFieldId, input);
	}
}

function addTag(indexId, docId, fieldId, tag){
	...
	var tagId = ajaxAddTagToField(indexId, docId, fieldId, tag);
	if (tagId == null){
		return;
	}
	addTagToDOM(tagId, tag);
}

function ajaxAddTagToField(indexId, docId, fieldId, tagValue){
	var options = {
	    method: "get",
...
function addTagToDOM(tagId, tagValue){
	var theDiv = $("tagValues");
	var span = document.createElement("span");
	span.id = "spanTag_" + tagId;
	var tagLink = document.createElement("a");
	tagLink.id = "anchor_tag_" + tagId;
	tagLink.href = "javascript:editTag(" + tagId + ",'" + tagValue + "')";
...
function loadTags(docId){
	gTaggingDocId = docId;
	var tags = ajaxLoadDocTagForDocument(gIndexId,docId);
	if (tags == null)
		return;
	$("tagDocId").innerHTML = docId;
	$("tagFieldId").innerHTML = gFieldId + " (" + getFieldNameFromId(gFieldId) + ")";
	$("tagValues").innerHTML = "";
...
function editTag(tagId, tagValue){	
	var input = prompt("Please enter new tag (hit cancel will delete the tag)",tagValue);
	if (input != null){
		if ( ajaxRemoveTag(tagId) ){
			addTag(gIndexId, gTaggingDocId, gFieldId, input);
		}
		else{
			alert("can not remove tag " + tagId);
		}
	}
	else {
		if ( ! ajaxRemoveTag(tagId) ){
