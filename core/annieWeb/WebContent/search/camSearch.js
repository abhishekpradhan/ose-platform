   ...
   '<h2 class="ostitle"> \
    ...
    <table border="0" cellpadding="0" cellspacing="0"> \
        <tr> \
            ...
            <nobr> \
            <a class="fl" target="frameWebpage" href="#{CACHE_URL}"> Cached</a> \
            </nobr></font>\
            </td> \
        </tr> \
    </table>');
...
/* javascript links */
function popUpFeatures(docId, featureId){
    var allItems = gRawResponseData["items"];
    for (var i = 0 ; i < allItems.length;i++){
        if (allItems[i]["DocId"] == docId){
            alert("Features : " + allItems[i]["Features"][featureId] + "\n"
                + "All : " + allItems[i]["Features"] + "\n" 
                + "Instance : " + allItems[i]["Instance"]);
            return;
        }
    }
}
