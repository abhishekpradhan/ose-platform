...
var SNIPPET_SERVLET_URL = "/annieWeb/SnippetServlet";
...
var gEnableSnippet = gQueryString.indexOf('snippet=yes') != -1; 
...
function init(){
    if (gQueryString.indexOf('index=') != -1){
        var p = gQueryString.indexOf('index=');
	gIndexId = 1 * gQueryString.substring(p+6)
        logMessage("Change index id to " + gIndexId)
    }
    initYUIComponents();
    renderMyPageTable();
...
var TitleTemplate = new Template(
   '<h2 class="ostitle">\
    ...
    <table border="0" cellpadding="0" cellspacing="0"> \
    	<tr> \
            <td class="j"><div id="divSnippet_#{DOCID}">#{SNIPPET}</div></td> \
        </tr> \
        <tr> \
            <td class="j">#{DESCRIPTION}<br> \
            </font>\
            </td> \
        </tr> \
    </table>\
    <nobr> \
    	<span class="a">#{DOMAIN} - </span>\
...
function renderMyPageTable() {
	var formatTitle = function(elCell, oRecord, oColumn, sData) { 
		...
		elCell.innerHTML = TitleTemplate.evaluate(
            ...
            "URL" : url,
			"SNIPPET" : oRecord.getData("Snippet"),
...
function selectSemYear(){
    var selectBox = $("selYear");
    var selectedOption = selectBox.options[selectBox.selectedIndex]
    $("YEAR").value = selectedOption.value; 
    
    selectBox = $("selSem");
    selectedOption = selectBox.options[selectBox.selectedIndex]
    $("SEM").value = selectedOption.value;
}
...
function getObjectQuery(){
	...
	for (var i = 0 ; i < fieldNames.length ; i++){
		var fieldName = fieldNames[i];
		if ( $(fieldName.toUpperCase()) ){
			query[fieldName] = $(fieldName.toUpperCase()).value.toLowerCase();
		}
	}
...
function showSnippets(){
	...
	var table = YAHOO.example.ClientPagination.myDataTable
	
	var docIds = new Array();
	var i = 0;
	var row = table.getFirstTrEl();
	while (row != null){
		var record = table.getRecord(row);
		docIds[i] = record.getData()["DocId"];
		i += 1;
		row = table.getNextTrEl(row);
	}
	...
	var domainId = $("inpDomainId").value;	
	options = { method:'get',
				parameters: {indexId : gIndexId, oquery : getObjectQuery(), domainId : domainId, docIds : docIds.join(",")},
	  			onSuccess: showSnippetsOnGUI,
	  			onFailure: function(){ alert('Something went wrong...') },
				};
	// YAHOO.example.container.wait.show();
	new Ajax.Request(SNIPPET_SERVLET_URL, options);
}

function showSnippetsOnGUI(obj){
	//YAHOO.example.container.wait.hide();
	var response = obj.responseText.evalJSON();
	if (response["numSnippetReturned"] == undefined){
		alert("Bad snippets");
		$("scratchBox").value = response + "";
	}
	var snippets = response["snippets"];
	for (var key in snippets){
		var snippetDiv = $("divSnippet_" + key);
		if (snippetDiv != undefined){
			snippetDiv.innerHTML = snippets[key];
			logMessage("Displayed snippet " + key + " " + snippets[key]); 
		}
	}
}
