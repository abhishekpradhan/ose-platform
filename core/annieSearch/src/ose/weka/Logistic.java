 ...
 */
public class Logistic extends Classifier implements OptionHandler,
	...
	public double[][] m_Par; //Kim : change this from private to public
	...
	public Enumeration listOptions() {
		...
		newVector.addElement(new Option("\tTurn on debugging output.", "D", 0,
				"-D"));
		newVector
				.addElement(new Option(
						"\tSet the ridge in the log-likelihood.", "R", 1,
						"-R <ridge>"));
		newVector.addElement(new Option(
				"\tSet the maximum number of iterations"
						+ " (default -1, until convergence).", "M", 1,
				"-M <number>"));
	...
	public String toString() {
		...
		String result = "Logistic Regression with ridge parameter of "
				+ m_Ridge;
		if (m_Par == null) {
		...
		for (int j = 1; j <= m_NumPredictors; j++) {
			...
			for (int k = 0; k < m_NumClasses - 1; k++) {
				...
				result += " "
						+ ((ORc > 1e10) ? "" + ORc : Utils.doubleToString(ORc,
								12, 4));
			}
			result += "\n";
		}
	...
	/**
	 * Kim hacked here 
	 * return a score, positive --> good, negative --> bad
	 */
	...
	public double getScore(Instance instance) {
		...
		double score = 0.0;
		for (int i = 0; i < instDat.length; i++) {
			score += m_Par[i][0] * instDat[i];	
		}
