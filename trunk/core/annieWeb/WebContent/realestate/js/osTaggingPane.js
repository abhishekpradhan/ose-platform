...
function addTag(fieldName, tag){
	...
	var tagToken;
	if (pos != -1 && pos >= tag.length - 3) {
		tagToken = tag.toLowerCase().replace(/,/g,""); //lower and remove "," for numbers like price		
	} else {
		pos = tag.lastIndexOf(',');
		if (pos != -1 && pos >= tag.length - 3) {
			tagToken = tag.toLowerCase().replace(/\./g,"");
			tagToken = tagToken.toLowerCase().replace(/,/,".");
		} else {
			tagToken = tag.toLowerCase().replace(/\./g,"");
			tagToken = tagToken.toLowerCase().replace(/,/g,"");
		}
	}
...
/*		
	var tagList = tag.split(/[^a-zA-Z0-9.,]/);
